...
def liste_mi(liste):
    # assert yap
    assert type(liste) == list, AssertionError('Verilen parametre list tipinde değil')

    # assert'ü geçmişse -> liste tipinde
    print(liste)
    pass
...
def listedeki_sayilar_toplami(liste):
    ...
    for i in liste:
        try:
            # bu eleman int mi değil mi -> assert
            assert int(i)
        except:
...
# Çözüm 6:
def hesap_makinesi():
    girdi = input("Hesaplamak istediğiniz işlemi girin: ")

    # önce elemanaları alalım ve girdi kontrolü yapalım
    elemanlar = girdi.split()
    if len(elemanlar) != 3:
        raise Exception('Araya boşluk bırakarak 3 eleman giriniz.')

    # işlem kontrolü
    islemler = ('+', '-', '*', '/')
    sembol = elemanlar[1]
    if not sembol in islemler:
        raise Exception('{0} işlemi, 4 işlemden biri değil'.format(sembol))

    # 1. ve 3. eleman float mu?
    try:
        sayi_1 = float(elemanlar[0])
        sayi_2 = float(elemanlar[2])
    except:
        raise Exception('Elemanlar sayısal olmalı.')
    else:
...
def bir_karakter_ver():
    ...
    karakter = input('Bir karakter ver: ')
    sonuc = ''
    ...
    try:
        ...
        # sayı değil
        try:
            # harf olma -> alphanumeric kontrolü
            assert karakter.isalpha()
